<h1 align="center">Hola, soy Javier Vásquez 👋</h1>
<h3 align="center">Software Developer · Full-Stack Developer</h3>

<p align="center">
  Construyo aplicaciones web, APIs y sistemas de gestión enfocados en resolver problemas reales,
  con especial interés en arquitectura de software, IoT y visión por computadora.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Angular-DD0031?style=flat-square&logo=angular&logoColor=white"/>
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white"/>
  <img src="https://img.shields.io/badge/C%23-239120?style=flat-square&logo=csharp&logoColor=white"/>
  <img src="https://img.shields.io/badge/.NET-512BD4?style=flat-square&logo=dotnet&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL%20Server-CC2927?style=flat-square&logo=microsoftsqlserver&logoColor=white"/>
  <img src="https://img.shields.io/badge/Supabase-3ECF8E?style=flat-square&logo=supabase&logoColor=white"/>
</p>

---

## 🧭 Sobre mí

- 💻 Desarrollador **Full-Stack**, con foco en sistemas mantenibles y escalables
- 🎓 Estudiante de **Ingeniería de Software**
- 🌐 Construcción de aplicaciones web, APIs REST y bases de datos
- 🔐 Autenticación, autorización y seguridad de aplicaciones
- 🤖 Interés activo en **IoT, Computer Vision e Inteligencia Artificial**
- 📚 En aprendizaje continuo sobre arquitectura de software, DevOps y buenas prácticas de ingeniería

**Filosofía de trabajo:**
> No se trata solo de escribir código, sino de construir soluciones que funcionen, se puedan mantener y resuelvan problemas reales.
>
> `Problema → Diseño → Arquitectura → Desarrollo → Pruebas → Despliegue → Mantenimiento`

---

## 🛠️ Stack tecnológico

**Frontend**
![Angular](https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white)
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Tailwind CSS](https://img.shields.io/badge/Tailwind_CSS-06B6D4?style=for-the-badge&logo=tailwindcss&logoColor=white)

**Backend**
![CSharp](https://img.shields.io/badge/C%23-239120?style=for-the-badge&logo=csharp&logoColor=white)
![.NET](https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white)

**Bases de datos**
![SQL Server](https://img.shields.io/badge/SQL%20Server-CC2927?style=for-the-badge&logo=microsoftsqlserver&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![Supabase](https://img.shields.io/badge/Supabase-3ECF8E?style=for-the-badge&logo=supabase&logoColor=white)

**Mobile & IoT**
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=for-the-badge&logo=flutter&logoColor=white)
![Arduino](https://img.shields.io/badge/Arduino-00979D?style=for-the-badge&logo=arduino&logoColor=white)
![ESP32](https://img.shields.io/badge/ESP32-E7352C?style=for-the-badge&logo=espressif&logoColor=white)

**IA & Computer Vision**
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![OpenCV](https://img.shields.io/badge/OpenCV-5C3EE8?style=for-the-badge&logo=opencv&logoColor=white)
`YOLO` · `EasyOCR` · `Image Processing`

**Herramientas**
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![GitHub](https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white)
![VS Code](https://img.shields.io/badge/VS%20Code-007ACC?style=for-the-badge&logo=visualstudiocode&logoColor=white)

---

## ⭐ Proyectos destacados

### 🚗 [UCE Parking](https://github.com/Euris1935/UCEPARKING-Panel.git)
Sistema inteligente de gestión de estacionamientos que integra software, IoT y visión por computadora.

`Flutter` `Angular` `Supabase` `ESP32` `Arduino` `Python` `YOLO` `EasyOCR`

- Reconocimiento de matrículas mediante Computer Vision
- Disponibilidad de espacios en tiempo real
- Registro de entrada/salida de vehículos y gestión de tickets
- App móvil, panel administrativo e integración con dispositivos IoT

### 💰 [PresuXcel](https://github.com/Javier06-10/PresuexcWeb.git)
Plataforma SaaS para la gestión y generación de presupuestos.

`Angular` `Supabase` `PostgreSQL` `Stripe` `TypeScript`

- Autenticación, control de acceso y gestión de usuarios
- Suscripciones y pagos integrados con Stripe
- Backend basado en servicios cloud

### 📱 [Celulares 360](https://github.com/Javier06-10/GestionCelulares.git)
Sistema de gestión para un negocio de dispositivos móviles.

`.NET` `C#` `Angular` `SQL Server`

- Módulos de inventario, productos, clientes y ventas
- Diseño de base de datos relacional y APIs REST
- Administración completa del sistema

---

## 🧠 Áreas de interés

```text
Software Engineering
├── Full-Stack Development
├── Software Architecture & Database Design
├── REST APIs & SaaS Development
├── Authentication & Authorization
├── Cloud Services
├── IoT
├── Computer Vision
└── DevOps
```

## 🤝 Conectemos

Abierto a colaborar en proyectos de desarrollo de software, aplicaciones web, APIs, SaaS, IoT y Computer Vision.

<p align="left">
  <a href="https://linkedin.com/in/TU_LINKEDIN" target="_blank">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="mailto:TU_EMAIL">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
  <a href="https://github.com/TU_USERNAME" target="_blank">
    <img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white"/>
  </a>
</p>

<p align="center"><i>Construyendo software. Aprendiendo constantemente. Resolviendo problemas reales.</i></p>
